...
async function loadBudgetData() {
    ...
    try {
        const response = await fetch(`${API_BASE}/budget?user_id=${currentUser.id}`);
        const result = await response.json();
...
async function addIncome() {
    ...
    if (!amountInput || !typeSelect) {
        showAlert('Form elementlari topilmadi!', 'error');
        return;
    }
    ...
    try {
        ...
        addIncomeToLocalStorage(amount, type);
    }
}
...
async function addExpense() {
    ...
    if (!categorySelect || !amountInput || !descriptionInput) {
        showAlert('Form elementlari topilmadi!', 'error');
        return;
    }
    ...
    try {
        ...
        showAlert('❌ Server bilan bog\'lanishda xatolik!', 'error');
        addExpenseToLocalStorage(category, amount, description);
    }
}
    ...
    expensesList.innerHTML = expenses.map(expense => `
        <div class="expense-item">
            <div class="expense-info">
                ...
                <div class="expense-date">${new Date(expense.date).toLocaleString('uz-UZ')}</div>
            </div>
...
function updateMonthFilter() {
    ...
    // Agar "Barcha yillar" tanlangan bo'lsa, barcha oylarni ko'rsatish
    if (selectedYear === 'all') {
        monthFilter.querySelectorAll('option').forEach(option => {
            option.style.display = '';
        });
    } else {
        // Faqat tanlangan yilda mavjud bo'lgan oylarni ko'rsatish
        const monthlyStats = JSON.parse(localStorage.getItem('monthlyStats') || '[]');
        const availableMonths = new Set();
        
        monthlyStats.forEach(stat => {
            if (stat.year == selectedYear) {
                availableMonths.add(stat.month);
            }
        });
        
        // Joriy oyni ham qo'shamiz
        const currentDate = new Date();
        if (currentDate.getFullYear() == selectedYear) {
            availableMonths.add(currentDate.getMonth());
        }
        
        // Optionlarni ko'rsatish/yashirish
        monthFilter.querySelectorAll('option').forEach((option, index) => {
            if (index === 0) return; // "Barcha oylar" optionini saqlab qolamiz
            
            const monthIndex = parseInt(option.value);
            if (availableMonths.has(monthIndex)) {
                option.style.display = '';
            } else {
...
function updateFilteredData() {
    ...
    // Arxivlangan va joriy xarajatlarni olish
    const archivedExpenses = JSON.parse(localStorage.getItem('archivedExpenses') || '[]');
    let filteredExpenses = [];
    
    // 1. Arxiv xarajatlarni qo'shish
    archivedExpenses.forEach(archive => {
        // Yil filteri
        if (selectedYear !== 'all' && archive.year != selectedYear) return;
        
        // Oy filteri
        if (selectedMonth !== 'all' && archive.month != selectedMonth) return;
        
        // Kategoriya filteri
        if (archive.expenses && archive.expenses.length > 0) {
            if (selectedCategory !== 'all') {
                const categoryExpenses = archive.expenses.filter(exp => 
                    exp.category === selectedCategory
                );
                filteredExpenses = filteredExpenses.concat(categoryExpenses);
            } else {
                filteredExpenses = filteredExpenses.concat(archive.expenses);
            }
        }
    });
    
    // 2. Joriy xarajatlarni qo'shish
    const currentDate = new Date();
    const currentMonth = currentDate.getMonth();
    const currentYear = currentDate.getFullYear();
    
    if ((selectedYear === 'all' || currentYear == selectedYear) && 
        (selectedMonth === 'all' || currentMonth == selectedMonth)) {
        
        let currentExpenses = expenses;
        if (selectedCategory !== 'all') {
            currentExpenses = expenses.filter(exp => exp.category === selectedCategory);
        }
        filteredExpenses = filteredExpenses.concat(currentExpenses);
    }
    
    console.log(`✅ ${filteredExpenses.length} ta xarajat topildi`);
...
// ==================== TUSHUMLAR MA'LUMOTLARI BO'LIMI (TUZATILGAN) ====================

// 1. Menyuga tugma qo'shamiz (asl showSection funksiyasini ishlatamiz)
function addTushumMenu() {
    const menu = document.querySelector('.menu');
    if (menu) {
        menu.innerHTML += `
            <button class="menu-btn" onclick="showSection('tushumSection')">
                <i class="fas fa-money-bill-wave"></i>
                <span>Tushum Ma'lumotlari</span>
            </button>
        `;
    }
}

// 2. Tushum bo'limini yaratamiz
function createTushumSection() {
    const mainContent = document.querySelector('.main-content');
    if (!mainContent) return;
    
    const tushumHTML = `
        <div id="tushumSection" class="section">
            <div class="tushum-header">
                <h1><i class="fas fa-money-bill-wave"></i> Tushum Ma'lumotlari</h1>
                <p class="subtitle">O'zingizning barcha daromad manbalaringiz haqida ma'lumot saqlang</p>
            </div>
            
            <div class="tushum-grid">
                <!-- 1-bo'lim: Ish haqi -->
                <div class="info-card">
                    <div class="card-header">
                        ...
                        <div class="card-actions">
                            <button class="btn-edit" onclick="editTushum('ishHaqi')" title="Tahrirlash">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn-delete" onclick="deleteTushum('ishHaqi')" title="O'chirish">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    </div>
                    <div class="card-content">
                        <textarea id="ishHaqiText" placeholder="Bu yerda ish haqingiz haqida ma'lumot yozing...
Misol:
• Ish joyi: Kompaniya nomi
• Oylik maosh: 5,000,000 so'm
• Qo'shimcha nafaqalar: Transport, ovqatlanish
• To'lov sanasi: Har oyning 10-kuni"></textarea>
                        <button class="btn-save" onclick="saveTushum('ishHaqi')">
                            <i class="fas fa-save"></i> Saqlash
                        </button>
                    </div>
                    <div class="saved-content" id="ishHaqiSaved"></div>
                </div>
                ...
                <div class="info-card">
                    <div class="card-header">
                        ...
                        <div class="card-actions">
                            <button class="btn-edit" onclick="editTushum('biznes')" title="Tahrirlash">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn-delete" onclick="deleteTushum('biznes')" title="O'chirish">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    </div>
                    <div class="card-content">
                        <textarea id="biznesText" placeholder="Bu yerda biznesingiz haqida ma'lumot yozing...
Misol:
• Biznes turi: Savdo, Xizmat ko'rsatish
• O'rtacha oylik daromad: 10,000,000 so'm
• Asosiy xarajatlar: Mahsulot, ijara, maosh
• Mijozlar soni: 50+"></textarea>
                        <button class="btn-save" onclick="saveTushum('biznes')">
                            <i class="fas fa-save"></i> Saqlash
                        </button>
                    </div>
                    <div class="saved-content" id="biznesSaved"></div>
                </div>
                
                <!-- 3-bo'lim: Boshqa tushumlar -->
                <div class="info-card">
                    <div class="card-header">
                        <div class="card-title">
                            <i class="fas fa-chart-line"></i>
                            <h3>Boshqa tushumlar</h3>
                        </div>
                        <div class="card-actions">
                            <button class="btn-edit" onclick="editTushum('boshqa')" title="Tahrirlash">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn-delete" onclick="deleteTushum('boshqa')" title="O'chirish">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    </div>
                    <div class="card-content">
                        <textarea id="boshqaText" placeholder="Bu yerda boshqa tushumlar haqida ma'lumot yozing...
Misol:
• Freelance ishlar: 2,000,000 so'm
• Investitsiyalar: 1,500,000 so'm
• Ijara daromadi: 3,000,000 so'm
• Qo'shimcha ishlar: 1,000,000 so'm"></textarea>
                        <button class="btn-save" onclick="saveTushum('boshqa')">
                            <i class="fas fa-save"></i> Saqlash
                        </button>
                    </div>
                    <div class="saved-content" id="boshqaSaved"></div>
                </div>
            </div>
            
            <!-- Statistika -->
            <div class="tushum-stats">
                <h3><i class="fas fa-chart-pie"></i> Umumiy ma'lumot</h3>
                <div class="stats-grid">
                    <div class="stat-item">
                        <div class="stat-icon">
                        ...
                        <div class="stat-info">
                            <h4>Saqlangan bo'limlar</h4>
                            <p id="savedCount">0/3</p>
                        </div>
                    </div>
                    <div class="stat-item">
                        <div class="stat-icon">
                        ...
                        <div class="stat-info">
                            ...
                            <p id="lastUpdate">Hali saqlanmagan</p>
                        </div>
                    </div>
                </div>
                <button class="btn-clear-all" onclick="deleteAllTushum()">
                    <i class="fas fa-trash-alt"></i> Barchasini o'chirish
                </button>
            </div>
        </div>
    `;
    
    mainContent.innerHTML += tushumHTML;
    addTushumStyles();
}

// 3. showSection funksiyasini yangilash (boshqa bo'limlarga o'tish uchun)
function showSection(sectionId) {
    ...
    // Barcha menyu tugmalarini faolsizlashtirish
    document.querySelectorAll('.menu-btn').forEach(btn => {
        btn.classList.remove('active');
    });
    ...
    if (targetSection) {
        ...
        // Agar tushum bo'limi bo'lsa, ma'lumotlarni yuklash
        if (sectionId === 'tushumSection') {
            setTimeout(() => {
                loadTushumData();
            }, 100);
        }
    ...
    // Tanlangan menyu tugmasini faollashtirish
    const currentEvent = window.event || arguments[0];
    if (currentEvent && currentEvent.target) {
        currentEvent.target.classList.add('active');
    }
    
    // Xarajatlar bo'limini yangilash
    if (sectionId === 'expenses') {
        setTimeout(() => {
            displayExpenses();
        }, 100);
    ...
    // Statistika bo'limini yangilash
...
// 4. Asosiy dasturni yangilash
function initializeAppWithTushum() {
    console.log('💰 Tushumlar bo\'limi qo\'shilmoqda...');
    ...
    autoLogin().then(() => {
        ...
        // Dastlab tushum bo'limini yashirish
        const tushumSection = document.getElementById('tushumSection');
        if (tushumSection) {
            tushumSection.style.display = 'none';
        }
...
// 5. Tushumlar uchun CSS
function addTushumStyles() {
    const style = document.createElement('style');
    style.textContent = `
        /* TUSHUM BO'LIMI STILLARI */
        #tushumSection {
            padding: 20px;
            max-width: 1400px;
            margin: 0 auto;
...
// 6. TUSHUM FUNKSIYALARI
let lastUpdateTime = null;

function saveTushum(type) {
    const textarea = document.getElementById(type + 'Text');
    const savedDiv = document.getElementById(type + 'Saved');
    
    if (!textarea || !savedDiv) return;
    
    const text = textarea.value.trim();
    
    if (!text) {
        showAlert('Iltimos, ma\'lumot kiriting!', 'error');
        return;
    }
    
    localStorage.setItem('tushum_' + type, text);
    lastUpdateTime = new Date();
    
    showSavedContent(type, text);
    updateStats();
    
    showAlert('✅ Ma\'lumot saqlandi!', 'success');
}

function showSavedContent(type, text) {
    const savedDiv = document.getElementById(type + 'Saved');
    const textarea = document.getElementById(type + 'Text');
    
    if (savedDiv && textarea) {
        if (text) {
            savedDiv.innerHTML = `<div class="content-box">${text.replace(/\n/g, '<br>')}</div>`;
...
function editTushum(type) {
    const savedText = localStorage.getItem('tushum_' + type);
    const textarea = document.getElementById(type + 'Text');
    const savedDiv = document.getElementById(type + 'Saved');
    
    if (savedText && textarea && savedDiv) {
        textarea.value = savedText;
        savedDiv.classList.remove('show');
        textarea.focus();
        showAlert('✏️ Tahrirlash uchun tayyor!', 'info');
    }
}

function deleteTushum(type) {
    const typeNames = {
        'ishHaqi': 'Ish haqi',
        'biznes': 'Biznes',
        'boshqa': 'Boshqa tushumlar'
    };
    
    if (!confirm(typeNames[type] + " ma'lumotlarini o'chirishni istaysizmi?")) {
        return;
    }
    
    localStorage.removeItem('tushum_' + type);
    ...
    updateStats();
    showAlert("🗑️ " + typeNames[type] + " ma'lumotlari o'chirildi", 'success');
}

function deleteAllTushum() {
    if (!confirm("Barcha tushum ma'lumotlarini o'chirishni istaysizmi?")) {
        return;
    }
    
    const types = ['ishHaqi', 'biznes', 'boshqa'];
    ...
    lastUpdateTime = null;
    updateStats();
    showAlert("🗑️ Barcha ma'lumotlar o'chirildi", 'success');
}
...
function updateStats() {
    ...
    types.forEach(type => {
        if (localStorage.getItem('tushum_' + type)) {
            savedCount++;
        }
    });
    ...
    if (savedCountElem) {
        savedCountElem.textContent = `${savedCount}/3`;
    }
    
    let updateText = 'Hali saqlanmagan';
    if (lastUpdateTime) {
        const now = new Date();
        const diff = Math.floor((now - lastUpdateTime) / 1000);
        
        if (diff < 60) {
            updateText = `${diff} soniya oldin`;
        } else if (diff < 3600) {
            updateText = `${Math.floor(diff / 60)} daqiqa oldin`;
        } else if (diff < 86400) {
            updateText = `${Math.floor(diff / 3600)} soat oldin`;
        } else {
            updateText = lastUpdateTime.toLocaleDateString('uz-UZ');
        }
    }
    
    if (lastUpdateElem) {
        lastUpdateElem.textContent = updateText;
    }
}

function loadTushumData() {
    const types = ['ishHaqi', 'biznes', 'boshqa'];
    
    types.forEach(type => {
        const savedText = localStorage.getItem('tushum_' + type);
        if (savedText) {
            showSavedContent(type, savedText);
        }
    });
...
// 7. DOM YUKLANISHI
document.addEventListener('DOMContentLoaded', function() {
    // Asl dasturni yangilash
    initializeApp = initializeAppWithTushum;
    
    // Dasturni ishga tushirish
    initializeApp();
});

console.log('✅ Tuzatilgan Tushum Ma\'lumotlari bo\'limi qo\'shildi!');